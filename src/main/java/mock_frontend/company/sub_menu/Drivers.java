package mock_frontend.company.sub_menu;

/**
 * FRONTEND CLASS
 *
 * Class contains pseudocode for the saved locations view option.
 *
 * Created by skeane on 3/10/2023.
 */
public class Drivers {

    // Displays all drivers in an interact-able table format.
    //          1. Make service/database call (Required info: Company ID).
    //          2. Selecting a location will allow for modification or deletion.
    //              a. Modification shows all fields.
    //              b. Option to cancel or save.
    //                  - Cancel: Go back to table view.
    //                  - Save: Call database/service.
    //          3. At the bottom of the table there should be an option to add a driver.
    //              a. Option should only appear if max number of drivers has not been hit.
    //              b. Option to cancel or save.
    //                  - Cancel: Go back to table view.
    //                  - Save: Call database/service.

}
